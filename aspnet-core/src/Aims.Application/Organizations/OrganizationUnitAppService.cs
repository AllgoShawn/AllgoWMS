...
namespace Aims.Organizations
{
    ...
    public class OrganizationUnitAppService : AimsAppServiceBase, IOrganizationUnitAppService
    {
        private readonly OrganizationUnitManager _organizationUnitManager;
        private readonly IRepository<OrganizationUnit, long> _organizationUnitRepository;
        private readonly IDapperRepository<OrganizationUnit, long> _organizationUnitDapperRepository;
        ...
        public OrganizationUnitAppService(
            ...
            IDapperRepository<OrganizationUnit, long> organizationUnitDapperRepository,
        ...
        */
        ...
        public async Task<OrganizationUnitDto> CreateOrganizationUnit(CreateOrganizationUnitInput input)
        {
            ...
            if (organizationUnits.Any(x => x.Code == input.Code))
            {
                throw new UserFriendlyException("There is already a workzone with store code " + input.Code);
            }
            ...
            string Query = $@"INSERT INTO AbpOrganizationUnits (CreationTime, CreatorUserId, IsDeleted, TenantId, ParentId, Code, DisplayName, Type) VALUES (GETDATE(), @UserId, 0, @TenantId, @ParentId, @Code, @DisplayName, @Type)";
            ...
            await _organizationUnitDapperRepository.ExecuteAsync(Query, para);

            var selectQuery = $@"SELECT TOP 1 Id FROM AbpOrganizationUnits WHERE IsDeleted = 0 AND TenantId = @TenantId AND Code = @Code;";
            ...
            var ou = _organizationUnitDapperRepository.Query(selectQuery, selectPara);

            organizationUnit.Id = ou.Select(i => i.Id).FirstOrDefault();
        ...
        public async Task<OrganizationUnitDto> UpdateOrganizationUnit(UpdateOrganizationUnitInput input)
        {
            ...
            string Query = $@"
                              UPDATE AbpOrganizationUnits SET DisplayName = @DisplayName, Type = @Type, LastModificationTime = GETDATE(), LastModifierUserId = @UserId WHERE Id = @Id AND TenantId = @TenantId;
                             ";
        ...
        private async Task<OrganizationUnitDto> CreateOrganizationUnitDto(OrganizationUnit organizationUnit)
        {
            ...
            dto.WarehouseCount = await _warehouseRepository.CountAsync(wh => wh.OrganizationUnitId == organizationUnit.Id);
            return dto;
        }
    }
}