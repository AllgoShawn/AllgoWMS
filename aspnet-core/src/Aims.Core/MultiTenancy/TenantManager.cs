...
using Aims.Editions;
using Aims.MultiTenancy.Demo;
using Abp.Extensions;
using Abp.Notifications;
using Abp.Runtime.Security;
using Microsoft.AspNetCore.Identity;
using Aims.Notifications;
using System;
...
namespace Aims.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
            ...
            string emailActivationLink)
        {
            ...
            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                ...
                using (_unitOfWorkManager.Current.SetTenantId(tenant.Id))
                {
                    ...
                    //20230211 Create Extra Static Role For New Tenant
                    var role = new Role(tenant.Id, "Warehouse") { IsDefault = false, IsStatic = true };
                    CheckErrors(await _roleManager.CreateAsync(role));

                    role = new Role(tenant.Id, "Store") { IsDefault = false, IsStatic = true };
                    CheckErrors(await _roleManager.CreateAsync(role));

                    role = new Role(tenant.Id, "Customer") { IsDefault = false, IsStatic = true };
                    CheckErrors(await _roleManager.CreateAsync(role));

                    role = new Role(tenant.Id, "Supplier") { IsDefault = false, IsStatic = true };
                    CheckErrors(await _roleManager.CreateAsync(role));

                    role = new Role(tenant.Id, "Transporter") { IsDefault = false, IsStatic = true };
                    CheckErrors(await _roleManager.CreateAsync(role));
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(AimsConsts.LocalizationSourceName).GetString("FreeEditionsCannotHaveTrialVersions");
